...
function show_basic_query(church_data, id) {
	...
	var bar = chart.selectAll("g")
		...
		.on("click", function(data) {
			...
			display_selected_churches(data, id);
		});
...
function display_selected_churches(data, id) {
	var selector = "#" + id + " .data-table";
	$(selector).empty();

	var keys = Object.keys(data[0]);
	keys.forEach(function(el) {
		$(selector).append("<th>" + el + "</th>\n");
	});
